...
from mangostar.utils import consistent_classes as const_class
...
def generate_nodes(tmp_env: Environment):
    ...
    template = tmp_env.get_template("ast_nodes.py.j2")
    rendered = template.render(node_list=node_list)
    formatted = format_result(rendered)
    store_file("nodes", formatted)
