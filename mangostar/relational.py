...
from mangostar import Visitor
from mangostar.circular import _init_tables
from mangostar.core import FlexibleModel
from mangostar.main import Component
from mangostar.main import Entity
from mangostar.main import System
from mangostar.matches import TableType
from mangostar.materialized import build_view
from mangostar.schema import PrePostSplit
...
class SQLVisitor(Visitor):
    ...
    def visit(self, _system: Optional[System] = None):
        ...
        sorted_topology = (
            list(filter(lambda x: x in split.post_nodes, topology))
        )
        for node_item in sorted_topology:
            ...
            child_table = self.node(
                node_item, parent_path = parent_child.json_path
            )
