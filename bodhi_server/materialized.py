...
def compilize(func, *args, **kw):
    ...
    cmp = fn.compile(dialect=psql.dialect(), compile_kwargs={"literal_binds": True})
    return cmp
...
def create_select(table: TableType, _adapter: MetaTableAdapter):
    ...
    select_stmt = select(traits_gen).where(_adapter.kernel.bucket == table.entity)
...
def gen_field():
    ...
    return {
        ...
        "attrs": {"node_type": "component", "sub_type": "field", "field_type": mark_of},
    }
